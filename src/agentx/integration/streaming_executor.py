    ...
    def execute_with_streaming(
        ...
        try:
            ...
            start_update = ProgressUpdate(
                type=ProgressType.STARTED.value, tool_name=tool_name, message=f"Starting {tool_name}"
            )
            ...
            if self._cancelled:
                cancel_update = ProgressUpdate(
                    type=ProgressType.CANCELLED.value, tool_name=tool_name, message="Execution cancelled"
                )
                if progress_callback:
            ...
            error_update = ProgressUpdate(
                type=ProgressType.ERROR.value, tool_name=tool_name, message=f"Error: {str(e)}"
            )
...
class ProgressTracker:
    ...
    def start_operation(self, operation_id: str, tool_name: str):
        ...
        with self._lock:
            self._operations[operation_id] = ProgressUpdate(type=ProgressType.STARTED.value, tool_name=tool_name)
...
class StreamingToolChain:
    ...
    def execute_chain(
        ...
        for i, (tool_name, arguments) in enumerate(self._chain):
            ...
            results = []
            for update in executor.execute_with_streaming(tool_executor, tool_name, arguments, chain_callback):
                yield update
