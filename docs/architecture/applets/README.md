...
| `context` | Partial | Mixed (surface multiplexing increases cognitive load) | Partial | No | Resolve ownership boundary between context-owned surfaces and first-class split applets, then reconcile naming/inventory docs. |
| `filesystem` | Yes (current scope) | Good | Mostly | No | Maintain viewport/select/action compatibility regression coverage. |
| `system-settings` | Partial | Good | Partial | No | Finalize dedicated runtime ownership and acceptance criteria parity against `PD-07` and `PD-18-AF-003`. |
...
- Applets requiring additional work: 2 of 6
