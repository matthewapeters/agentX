...
tmux select-pane -t "$STORIES_PANE" -T stories
tmux select-pane -t "$LIVE_PANE" -T live-core
...
STORIES_RECORD="$(grep '|stories|' <<< "$PANE_META" || true)"
LIVE_RECORD="$(grep '|live-core|' <<< "$PANE_META" || true)"
CTRL_RECORD="$(grep '|controller|' <<< "$PANE_META" || true)"

if [[ -z "$STORIES_RECORD" || -z "$LIVE_RECORD" || -z "$CTRL_RECORD" ]]; then
  echo "FAIL: Missing one or more titled panes (stories/live-core/controller)"
  exit 1
fi
...
if [[ "$LIVE_TOP" -ne 0 || "$LIVE_LEFT" -le "$STORIES_LEFT" ]]; then
  echo "FAIL: Live-core pane must occupy right column"
  exit 1
fi

if [[ "$CTRL_ACTIVE" -ne 1 ]]; then
  echo "FAIL: Controller pane should be active for prompt input"
  exit 1
fi
