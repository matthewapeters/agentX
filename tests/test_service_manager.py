...
class TestServiceConfig(unittest.TestCase):
    ...
    def test_service_config_creation(self):
        ...
        config = ServiceConfig(name="Ollama", host="localhost", port=11435, health_endpoint="/api/models")
    ...
    def test_service_config_url(self):
        ...
        config = ServiceConfig(name="Test", host="example.com", port=8080, health_endpoint="/health")
...
class TestHealthCheck(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        self.config = {"agentx": {"ollama_host": "localhost:11435"}, "agentix": {"host": "localhost:8000"}}
...
class TestServiceStartup(unittest.TestCase):
    ...
    def test_start_service_no_command(self, mock_check):
        ...
        service = ServiceConfig(name="NoStart", host="localhost", port=9999, health_endpoint="/health")
...
class TestServiceStartupErrorPaths(unittest.TestCase):
    """Coverage uplift for error paths in start_service."""
    ...
    def test_start_service_unknown_service_name(self):
        """Unknown service_name → return False immediately (line 144)."""
        result = self.manager.start_service("does_not_exist")
        self.assertFalse(result)

    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen", side_effect=FileNotFoundError("ollama: not found"))
    def test_start_service_file_not_found(self, mock_popen, mock_health):
        """FileNotFoundError when start_command binary is missing → return False."""
        result = self.manager.start_service("ollama", timeout=1)
        self.assertFalse(result)

    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_process_exits_prematurely(self, mock_popen, mock_health):
        """Process crashes before becoming healthy → return False."""
        mock_process = Mock()
        mock_process.poll.return_value = 1  # non-None → exited
        stderr_data = b"some error message"
    ...
    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_process_exits_prematurely_no_stderr(self, mock_popen, mock_health):
        """Process crashes with no stderr output → return False."""
        mock_process = Mock()
        mock_process.poll.return_value = 1  # non-None → exited
    ...
    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_running_but_unhealthy_at_timeout(self, mock_popen, mock_health):
        """Process is still running but never becomes healthy → log and return False."""
        mock_process = Mock()
        mock_process.poll.return_value = None  # still running
        mock_popen.return_value = mock_process

        # Use a tiny timeout so we exit quickly
        result = self.manager.start_service("ollama", timeout=0)
        self.assertFalse(result)

    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_crashed_at_timeout_with_stderr(self, mock_popen, mock_health):
        """Process crashes at timeout boundary, with stderr output logged."""
        mock_process = Mock()
        # poll returns None initially (so we exit the while loop and hit the timeout block)
        # then poll() returns non-None in the timeout check
        mock_process.poll.side_effect = [None, None, 2]
        mock_process.communicate.return_value = (b"", b"startup error details")
        mock_popen.return_value = mock_process
    ...
    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen", side_effect=Exception("unexpected popen error"))
    def test_start_service_popen_generic_exception(self, mock_popen, mock_health):
        """Generic exception from Popen → log and return False."""
        result = self.manager.start_service("ollama", timeout=1)
        self.assertFalse(result)

    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_premature_exit_communicate_raises(self, mock_popen, mock_health):
        """If communicate() raises during premature-exit stderr capture, continue gracefully."""
        mock_process = Mock()
        mock_process.poll.return_value = 1  # exited
        mock_process.communicate.side_effect = Exception("communicate error")
        mock_popen.return_value = mock_process
    ...
    @patch("agentx.service_manager.ServiceManager.check_health", return_value=False)
    @patch("subprocess.Popen")
    def test_start_service_timeout_crashed_with_stderr(self, mock_popen, mock_health):
        """At timeout, crashed process with stderr → stderr gets logged (lines 203-204)."""
        mock_process = Mock()
        # poll() returns non-None at the final timeout check (process crashed)
        mock_process.poll.return_value = 2
        mock_process.communicate.return_value = (b"", b"startup failed with trace")
        mock_popen.return_value = mock_process
    ...
    def test_shutdown_with_exception(self):
        """Exception during terminate is caught and logged."""
        mock_process = Mock()
        mock_process.poll.return_value = None  # running
        mock_process.terminate.side_effect = Exception("terminate exploded")
        self.manager._started_processes.append(mock_process)
