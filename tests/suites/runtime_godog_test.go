...
func initializeAll(sc *godog.ScenarioContext) {
	...
	runtimesteps.InitializeTransactionWrite(sc)
	runtimesteps.InitializeTypeValidation(sc)
	runtimesteps.InitializeNormalization(sc)
	runtimesteps.InitializeConfigWrite(sc)
	runtimesteps.InitializeConfigWatch(sc)
