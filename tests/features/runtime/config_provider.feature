...
Feature: Provider configuration resolution
  ...
  Scenario: Default provider is ollama when unset
    Given no provider deployment config exists
    When the provider configuration is resolved
    Then the effective provider is "ollama"

  # use-case: UC-CONFIG-PROVIDER-VALUE
  Scenario: An explicit provider value is honored
    Given a provider deployment config with provider "llamacpp"
  ...
  Scenario: An ollama provider resolves the ollama host and model
    Given a provider deployment config with provider "ollama"
  ...
  Scenario: chat_backend is accepted as a provider alias
    Given a provider deployment config with chat_backend "llamacpp"
  ...
  Scenario: First run seeds provider into the deployment config
    Given no provider deployment config exists
    When the provider configuration is resolved with provider "llamacpp"
    Then a provider deployment config file is created
    And the seeded provider deployment config contains provider "llamacpp"
